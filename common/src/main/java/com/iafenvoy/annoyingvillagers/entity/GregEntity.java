...
public class GregEntity extends HostileEntity {
    ...
    public GregEntity(EntityType<GregEntity> type, World world) {
        super(type, world);
        this.setStepHeight(0.6f);
        this.experiencePoints = 0;
        this.setAiDisabled(false);
        this.setCustomName(Text.literal("Greg"));
        ...
        }
        {
            ...
            for (Entity entityiterator : _entfound) {
                if (this.getTarget() == entityiterator) {
                    if (attack == 1) {
                        ItemStack stack = new ItemStack(Items.GOLDEN_APPLE).copy();
                        stack.setCount(1);
                        this.setStackInHand(Hand.MAIN_HAND, stack);
                        Runnable runnable = () -> {
                            if (world instanceof World _level)
